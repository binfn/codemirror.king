...
class DummyServer {
    ...
    broadcast(client) {
        if (this.delayed.indexOf(client) > -1)
            return;
        this.sync(client);
        this.send(client);
    ...
    type(client, text, pos = this.states[client].selection.main.head) {
...
describe("collab", () => {
    ...
    it("supports shared effects", () => {
        ...
        let marks = StateField.define({
            create: () => [],
            update(value, tr) {
                value = value.map(m => m.map(tr.changes)).filter(x => x);
