import ist from "ist";
import { EditorState, Facet, Prec, StateField, StateEffect } from "@codemirror/state";
function mk(...extensions) {
    return EditorState.create({ extensions });
}
let num = Facet.define(), str = Facet.define(), bool = Facet.define();
describe("EditorState facets", () => {
    ...
    it("includes sub-extenders", () => {
        let e = (s) => [num.of(s.length), num.of(+s)];
    ...
    it("sorts extensions by priority", () => {
        let st = mk(str.of("a"), str.of("b"), Prec.high(str.of("c")), Prec.highest(str.of("d")), Prec.low(str.of("e")), Prec.high(str.of("f")), str.of("g"));
    ...
    it("can handle dependencies on facets that aren't present in the state", () => {
        let st = mk(num.compute([str], s => s.facet(str).join().length), str.compute([bool], s => s.facet(bool).toString()));
    ...
    it("works with a static combined facet", () => {
        let f = Facet.define({ combine: ns => ns.reduce((a, b) => a + b, 0) });
    ...
    it("applies effects from reconfiguring transaction to new fields", () => {
        let st = mk();
        let effect = StateEffect.define();
        let field = StateField.define({
            create(state) {
                var _a;
                return (_a = state.facet(num)[0]) !== null && _a !== void 0 ? _a : 0;
            },
            update(val, tr) {
    ...
    it("errors on cyclic dependencies", () => {
        ist.throws(() => mk(num.compute([str], s => s.facet(str).length), str.compute([num], s => s.facet(num).join())), /cyclic/i);
    });
