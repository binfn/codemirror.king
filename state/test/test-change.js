...
function rStr(l) {
    let result = "";
    for (let i = 0; i < l; i++)
        result += String.fromCharCode(97 + r(26));
    return result;
}
function rChange(len) {
    if (len == 0 || r(3) == 0)
        return { insert: rStr(r(5) + 1), from: r(len) };
    let from = r(len - 1);
    return { from, to: Math.min(from + r(5) + 1, len), insert: r(2) == 0 ? rStr(r(2) + 1) : undefined };
}
function rChanges(len, count) {
    let result = [];
    for (let i = 0; i < count; i++)
        result.push(rChange(len));
    return result;
}
describe("ChangeDesc", () => {
    describe("composition", () => {
        ...
        it("can compose unrelated changes", () => comp("5 0:2", "1 2:0 4", "1 2:0 2 0:2"));
        it("cancels insertions with deletions", () => comp("2 0:2 2", "2 2:0 2", "4"));
        it("joins adjacent insertions", () => comp("2 0:2 2", "4 0:3 2", "2 0:5 2"));
        it("joins adjacent deletions", () => comp("2 5:0", "1 1:0", "1 6:0"));
        it("allows a delete to shadow multiple operations", () => comp("2 2:0 0:3", "5:0", "4:0"));
        it("can handle empty sets", () => comp("", "0:8", "8:0", "", ""));
    ...
    describe("mapping", () => {
        function over(a, b, result) {
            ist(String(mk(a).mapDesc(mk(b))), result);
        }
        function under(a, b, result) {
            ist(String(mk(a).mapDesc(mk(b), true)), result);
        }
        it("can map over an insertion", () => over("4 0:1", "0:3 4", "7 0:1"));
        it("can map over a deletion", () => over("4 0:1", "2:0 2", "2 0:1"));
        ...
        it("can handle changes after", () => over("0:1 2:0 8", "6 1:0 0:5 3", "0:1 2:0 12"));
        it("joins deletions", () => over("5:0 2 3:0 2", "4 4:0 4", "6:0 2"));
    ...
    describe("mapPos", () => {
        function map(spec, ...cases) {
            let set = mk(spec);
            for (let [from, to, opt] of cases) {
                let assoc = typeof opt == "number" ? opt : -1;
                let mode = { D: MapMode.TrackDel, A: MapMode.TrackAfter, B: MapMode.TrackBefore }[opt];
                ist(set.mapPos(from, assoc, mode), to);
            }
        }
        it("maps through an insertion", () => map("4 0:2 4", [0, 0], [4, 4], [4, 6, 1], [5, 7], [8, 10]));
        it("maps through deletion", () => map("4 4:0 4", [0, 0], [4, 4], [4, 4, "D"], [4, 4, "B"], [4, null, "A"], [5, 4], [5, null, "D"], [5, null, "B"], [5, null, "A"], [7, 4], [8, 4], [8, 4, "D"], [8, null, "B"], [8, 4, "A"], [9, 5], [12, 8]));
        it("maps through multiple insertions", () => map("0:2 2 0:2 2 0:2", [0, 0], [0, 2, 1], [1, 3], [2, 4], [2, 6, 1], [3, 7], [4, 8], [4, 10, 1]));
        it("maps through multiple deletions", () => map("2:0 2 2:0 2 2:0", [0, 0], [1, 0], [2, 0], [3, 1], [4, 2], [5, 2], [6, 2], [7, 3], [8, 4], [9, 4], [10, 4]));
        it("maps through mixed edits", () => map("2 0:2 2:0 0:2 2 2:0 0:2", [0, 0], [2, 2], [2, 4, 1], [3, 4], [4, 4], [4, 6, 1], [5, 7], [6, 8], [7, 8], [8, 8], [8, 10, 1]));
        it("stays on its own side of replacements", () => map("2 2:2 2", [2, 2, 1], [2, 2, -1], [2, 2, "D"], [2, 2, "B"], [2, null, "A"], [3, 2, -1], [3, 4, 1], [3, null, "D"], [3, null, "B"], [3, null, "A"], [4, 4, 1], [4, 4, -1], [4, 4, "D"], [4, null, "B"], [4, 4, "A"]));
        it("maps through insertions around replacements", () => map("0:1 2:2 0:1", [0, 0, -1], [0, 1, 1], [1, 1, -1], [1, 3, 1], [2, 3, -1], [2, 4, 1]));
        it("stays in between replacements", () => map("2:2 2:2", [2, 2, -1], [2, 2, 1]));
    });
});
describe("ChangeSet", () => {
    ...
    it("can be iterated", () => {
        ...
        ist(JSON.stringify(result), JSON.stringify([[4, 4, 4, 6, "ok"], [6, 8, 8, 8, ""]]));
        result = [];
        set.iterGaps((fromA, toA, len) => result.push([fromA, toA, len]));
        ist(JSON.stringify(result), JSON.stringify([[0, 0, 4], [4, 6, 2], [8, 8, 2]]));
    });
    ...
    it("mapping still converges when mapping through multiple changes", () => {
        for (let i = 0, total = 100; i < total; i++) {
            ...
            let doc = Text.of([rStr(size)]);
            ist(a.compose(b).compose(c$ab).apply(doc).toString(), c.compose(a$c).compose(b$ca).apply(doc).toString());
        }
    });
    ...
    it("survives random sequences of changes", () => {
        for (let i = 0; i < 50; i++) {
            let doc = doc10, txt = doc.toString(), all = [], inv = [];
            let log = [];
            try {
                for (let j = 0; j < 50; j++) {
                    let set, change = rChange(doc.length);
