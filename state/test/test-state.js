...
describe("EditorState", () => {
    ...
    const someAnnotation = Annotation.define();
    ...
    it("stores and updates fields", () => {
        let field1 = StateField.define({ create: () => 0, update: val => val + 1 });
        let field2 = StateField.define({ create: state => state.field(field1) + 10, update: val => val });
    ...
    it("can replace extension groups", () => {
        let comp = new Compartment, f = Facet.define(), content = f.of(10);
    ...
    it("raises an error on duplicate extension groups", () => {
        let comp = new Compartment, f = Facet.define();
        ist.throws(() => EditorState.create({ extensions: [comp.of(f.of(1)), comp.of(f.of(2))] }), /duplicate use of compartment/i);
        ist.throws(() => EditorState.create({ extensions: comp.of(comp.of(f.of(1))) }), /duplicate use of compartment/i);
    });
    it("preserves compartments on reconfigure", () => {
        let comp = new Compartment, f = Facet.define(), init = comp.of(f.of(10));
        ...
        it("can constrain the selection", () => {
            let state = EditorState.create({
                extensions: EditorState.transactionFilter.of(tr => {
                    if (tr.selection && tr.selection.main.to > 4)
                        return [tr, { selection: { anchor: 4 } }];
                    else
                        return tr;
                }),
                doc: "one two"
