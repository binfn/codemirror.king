import { classHighlightStyle, HighlightStyle, styleTags, tags as t, highlightTree } from "@codemirror/highlight";
...
function test(name, spec, { parse = parser, highlight = classHighlightStyle.match } = {}) {
    it(name, () => {
        let { content, tokens } = parseSpec(spec[0] == "\n" ? spec.slice(1) : spec);
        let tree = parse.parse(content), emit = [];
...
describe("highlighting", () => {
    ...
    test("can highlight overlays", `[punctuation:{]...[punctuation:{][string:"foo][punctuation:}]..[punctuation:{][string:bar"] [variableName:x][punctuation:}].[punctuation:}]`, { parse: wrapper });
});
