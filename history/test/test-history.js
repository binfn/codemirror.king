import ist from "ist";
import { EditorState, EditorSelection, Transaction, StateEffect, StateField } from "@codemirror/state";
import { isolateHistory, history, redo, redoDepth, redoSelection, undo, undoDepth, undoSelection, invertedEffects, historyField } from "@codemirror/history";
function mkState(config, doc) {
    return EditorState.create({
        extensions: [history(config), EditorState.allowMultipleSelections.of(true)],
        doc
    });
}
function type(state, text, at = state.doc.length) {
    return state.update({ changes: { from: at, insert: text } }).state;
}
function timedType(state, text, atTime) {
    return state.update({ changes: { from: state.doc.length, insert: text },
        annotations: Transaction.time.of(atTime) }).state;
}
function receive(state, text, from, to = from) {
    return state.update({ changes: { from, to, insert: text },
        annotations: Transaction.addToHistory.of(false) }).state;
}
function command(state, cmd, success = true) {
    ist(cmd({ state, dispatch(tr) { state = tr.state; } }), success);
    return state;
}
describe("history", () => {
    ...
    describe("effects", () => {
        it("includes inverted effects in the history", () => {
            ...
            let invert = invertedEffects.of(tr => {
                for (let e of tr.effects)
                    if (e.is(set))
                        return [set.of(tr.startState.field(field))];
                return [];
            });
        ...
        class Comment {
            ...
            eq(other) { return this.from == other.from && this.to == other.to && this.text == other.text; }
        }
        function mapComment(comment, mapping) {
        ...
        let addComment = StateEffect.define({ map: mapComment });
        let rmComment = StateEffect.define({ map: mapComment });
        let comments = StateField.define({
            create: () => [],
            update(value, tr) {
                value = value.map(c => mapComment(c, tr.changes)).filter(x => x);
                for (let effect of tr.effects) {
                    if (effect.is(addComment))
                        value = value.concat(effect.value);
                    else if (effect.is(rmComment))
                        value = value.filter(c => !c.eq(effect.value));
                }
        ...
        let invertComments = invertedEffects.of(tr => {
            ...
            for (let effect of tr.effects) {
                if (effect.is(addComment) || effect.is(rmComment)) {
                    ...
                    if (src)
                        effects.push((effect.is(addComment) ? rmComment : addComment).of(src));
                }
            }
            for (let comment of tr.startState.field(comments)) {
                if (!mapComment(comment, tr.changes))
                    effects.push(addComment.of(comment));
            }
            return effects;
        });
        function commentStr(state) { return state.field(comments).map(c => c.text + "@" + c.from).join(","); }
        it("can map effects", () => {
