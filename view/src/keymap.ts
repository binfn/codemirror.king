...
import {Command} from "./extension.ts"
import {modifierCodes} from "./input.ts"
import {base, keyName} from "../../w3c-keyname/index.ts"
import {Facet, EditorState} from "../../state/src/index.ts"
...
function buildKeymap(bindings: readonly KeyBinding[], platform = currentPlatform) {
  ...
  const add = (scope: string, key: string, command: Command, preventDefault?: boolean) => {
    ...
    const binding = scopeObj[full] || (scopeObj[full] = {preventDefault: false, commands: []})
  ...
  for (const b of bindings) {
    ...
    if (!name) continue
    for (const scope of b.scope ? b.scope.split(" ") : ["editor"]) {
...
function runHandlers(map: Keymap, event: KeyboardEvent, view: EditorView, scope: string): boolean {
  ...
  const runFor = (binding: Binding | undefined) => {
    if (binding) {
      for (const cmd of binding.commands) if (cmd(view)) return true
  ...
  if (scopeObj) {
    ...
    if (isChar && (event.shiftKey || event.altKey || event.metaKey) &&
      (baseName = base[event.code]) && baseName != name) { //TERMY
