...
describe("DOM changes", () => {
    ...
    it("supports deleting lines", () => {
        let cm = tempView("1\n2\n3\n4\n5\n6");
        for (let i = 0, lineDOM = cm.domAtPos(0).node.parentNode; i < 4; i++)
            lineDOM.childNodes[1].remove();
        flush(cm);
    ...
    it("can deal with large insertions", () => {
        ...
        for (let i = 0, lineDOM = cm.domAtPos(0).node.parentNode; i < 100; i++)
            lineDOM.appendChild(node.cloneNode(true));
        flush(cm);
    ...
    it("correctly handles changes ending on a widget", () => {
        ...
        let field = StateField.define({
            create() {
                return Decoration.set([Decoration.widget({ widget }).range(2),
