import { Decoration, WidgetType, BlockType, __test } from "@codemirror/view";
...
describe("HeightMap", () => {
    ...
    function mk(text, deco = []) {
        return HeightMap.empty().applyChanges([Decoration.set(deco, true)], Text.empty, o(text), [new ChangedRange(0, 0, 0, text.length)]);
    }
    ...
    it("separates lines with decorations on them", () => {
        let map = mk(doc(10, 10, 20, 5), [Decoration.widget({ widget: new MyWidget(20) }).range(5),
            Decoration.replace({}).range(25, 46)]);
        ist(map.length, 48);
        ist(map.toString(), "line(10:20) gap(10) line(26-21)");
    });
    it("ignores irrelevant decorations", () => {
        let map = mk(doc(10, 10, 20, 5), [Decoration.widget({ widget: new NoHeightWidget }).range(5),
    ...
    it("updates the length of replaced decorations for changes", () => {
        ...
        map = map.applyChanges([Decoration.set(Decoration.replace({}).range(5, 10))], text, o(text.replace(7, 12, Text.empty)), [new ChangedRange(7, 12, 7, 7)]);
    ...
    it("stores information about block widgets", () => {
        ...
        ist(type.map(b => b.height).join(), [10, 13, oracle.lineHeight, 5].join());
    ...
    it("properly shrinks gaps when partially replaced", () => {
        ...
        map = map.applyChanges([Decoration.set(Decoration.replace({}).range(15, 55))], text, o(text), [new ChangedRange(15, 55, 15, 55)]);
    ...
    describe("blockAt", () => {
        it("finds blocks in a gap", () => {
            ...
            ist(block2.from, 4);
            ist(block2.to, 7);
            ist(block2.top, block1.bottom);
            ist(block2.bottom, block1.bottom, ">");
        ...
        it("finds widget blocks", () => {
            ...
            ist(block2.from, 4);
            ist(block2.to, 4);
            ist(block2.top, block1.height);
            ist(block2.height, 100);
