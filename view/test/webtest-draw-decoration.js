...
import ist from "ist";
const filterDeco = StateEffect.define();
const addDeco = StateEffect.define();
function text(node) {
    return (node.textContent || "").replace(/\u200b/g, "");
}
function decos(startState = Decoration.none) {
    let field = StateField.define({
        create() { return startState; },
        update(value, tr) {
            value = value.map(tr.changes);
            for (let effect of tr.effects) {
                if (effect.is(addDeco))
                    value = value.update({ add: effect.value });
                else if (effect.is(filterDeco))
...
describe("EditorView decoration", () => {
    it("renders tag names", () => {
        ...
        ist(cm.contentDOM.innerHTML.replace(/<\/?div.*?>/g, "|"), "|on<em>e</em>||<em>t</em>wo|");
    });
    ...
    it("updates for removed decorations", () => {
        ...
        cm.dispatch({ effects: filterDeco.of((from) => from == 4) });
    ...
    class WordWidget extends WidgetType {
        ...
        eq(other) { return this.word.toLowerCase() == other.word.toLowerCase(); }
        toDOM() {
    ...
    describe("widget", () => {
        ...
        it("places the cursor based on side", () => {
            let cm = requireFocus(decoEditor("abc", [w(2, new WordWidget("A"), -1),
        ...
        it("can remove widgets at the end of a line", () => {
    ...
    function r(from, to, spec = {}) { return Decoration.replace(spec).range(from, to); }
    describe("replaced", () => {
        ...
        it("allows replacing a single replaced range with two adjacent ones", () => {
            ...
            ist(cm.contentDOM.firstChild.querySelectorAll("span").length, 2);
        });
        ...
        it("raises errors for replacing decorations from plugins if they cross lines", () => {
            ist.throws(() => {
                tempView("one\ntwo", [ViewPlugin.fromClass(class {
                        constructor() {
                            this.deco = Decoration.set(Decoration.replace({ widget: new WordWidget("ay") }).range(2, 5));
                        }
                    }, {
    ...
    describe("line attributes", () => {
        function classes(cm, ...lines) {
            for (let i = 0; i < lines.length; i++) {
                let className = cm.contentDOM.childNodes[i].className.split(" ")
        ...
        it("updates when line attributes are removed", () => {
            ...
            let cm = decoEditor("foo\nbar", ds);
            cm.dispatch({ effects: filterDeco.of((_f, _t, deco) => !ds.slice(1).some(r => r.value == deco)) });
            classes(cm, "a", "");
        });
    ...
    function bw(pos, side = -1, name = "n") {
        return Decoration.widget({ widget: new BlockWidget(name), side, block: true }).range(pos);
    }
    function br(from, to, name = "r", inclusive) {
        return Decoration.replace({ widget: new BlockWidget(name), inclusive, block: true }).range(from, to);
    }
    function widgets(cm, ...groups) {
        let found = [[]];
        for (let n = cm.contentDOM.firstChild; n; n = n.nextSibling) {
            if (n.nodeName == "HR")
                found[found.length - 1].push(n.getAttribute("data-name"));
            else
                found.push([]);
        }
    ...
    describe("block widgets", () => {
        ...
        it("removes widgets when they vanish", () => {
            ...
            widgets(cm, ["A"], ["B", "C"], ["D"]);
            cm.dispatch({ effects: filterDeco.of((_f, _t, deco) => deco.spec.side < 0) });
            widgets(cm, ["A"], ["C"], []);
        });
        ...
        it("doesn't redraw unchanged widgets", () => {
            ...
            cm.dispatch({ effects: [
                    filterDeco.of((_f, _t, deco) => deco.spec.side < 0),
                    addDeco.of([bw(7, 1, "B")])
                ] });
            widgets(cm, ["A"], [], ["B"]);
        ...
        it("raises an error when providing block widgets from plugins", () => {
            ist.throws(() => {
                tempView("abc", [ViewPlugin.fromClass(class {
                        constructor() {
                            this.deco = Decoration.set(Decoration.replace({ widget: new BlockWidget("oh"), block: true }).range(1, 2));
                        }
                    }, {
                        decorations: o => o.deco
                    })]);
