import { Range, RangeSet, RangeValue } from "@codemirror/rangeset";
import { ChangeSet } from "@codemirror/state";
import ist from "ist";
class Value extends RangeValue {
    constructor(spec = {}, empty) {
        super();
    ...
    static names(v) {
        let result = [];
        for (let val of v)
            if (val.name || val.point)
                result.push(val.name || "POINT");
        return result.join("/");
    }
}
function cmp(a, b) { return a.from - b.from; }
function mk(from, to, spec) {
    if (typeof to != "number") {
        spec = to;
        to = from;
    }
...
function mkSet(ranges) { return RangeSet.of(ranges); }
function changeSet(changes) {
    let collect = [];
    for (let [from, to, len] of changes) {
        if (len)
            collect.push({ insert: "x".repeat(len), from, to });
        if (from < to)
            collect.push({ from, to });
    }
...
describe("RangeSet", () => {
    ...
    describe("map", () => {
        function test(positions, changes, newPositions) {
            let set = mkSet(positions);
            let mapped = set.map(changeSet(changes));
            let out = [];
            for (let iter = mapped.iter(); iter.value; iter.next())
                out.push(iter.from + "-" + iter.to);
            ist(JSON.stringify(out), JSON.stringify(newPositions.map(p => Array.isArray(p) ? p[0] + "-" + p[1] : p + "-" + p)));
        }
        it("can map through changes", () => test([mk(1), mk(4), mk(10)], [[0, 0, 1], [2, 3, 0], [8, 8, 20]], [2, 4, 30]));
        it("takes inclusivity into account", () => test([mk(1, 2, { startSide: -1, endSide: 1 })], [[1, 1, 2], [2, 2, 2]], [[1, 6]]));
        it("uses side to determine mapping of points", () => test([mk(1, 1, { startSide: -1, endSide: -1 }), mk(1, 1, { startSide: 1, endSide: 1 })], [[1, 1, 2]], [1, 3]));
        it("defaults to exclusive on both sides", () => test([mk(1, 2)], [[1, 1, 2], [4, 4, 2]], [[3, 4]]));
        it("drops point ranges", () => test([mk(1, 2)], [[1, 2, 0], [1, 1, 1]], []));
        it("drops ranges in deleted regions", () => test([mk(1, 2), mk(3)], [[0, 4, 0]], []));
        it("shrinks ranges", () => test([mk(2, 4), mk(2, 8), mk(6, 8)], [[3, 7, 0]], [[2, 3], [2, 4], [3, 4]]));
        it("leaves point ranges on change boundaries", () => test([mk(2), mk(4)], [[2, 4, 6]], [2, 8]));
    ...
    class Comparator {
        ...
        addRange(from, to) {
            if (this.ranges.length && this.ranges[this.ranges.length - 1] == from)
                this.ranges[this.ranges.length - 1] = to;
            else
                this.ranges.push(from, to);
        }
        compareRange(from, to) { this.addRange(from, to); }
        comparePoint(from, to) { this.addRange(from, to); }
    }
    describe("compare", () => {
        function test(ranges, update, changes) {
            ...
            if (update.changes)
                newSet = newSet.map(docChanges);
            if (update.filter || update.add)
                newSet = newSet.update(update);
            if (update.prepare)
                update.prepare(newSet);
            let comp = new Comparator;
            RangeSet.compare([set], [newSet], docChanges, comp);
            ist(JSON.stringify(comp.ranges), JSON.stringify(changes));
        }
        it("notices added ranges", () => test([mk(2, 4, "a"), mk(8, 11, "a")], {
            add: [mk(3, 9, "b"), mk(106, 107, "b")]
        }, [3, 9, 106, 107]));
        it("notices deleted ranges", () => test([mk(4, 6, "a"), mk(5, 7, "b"), mk(6, 8, "c"), mk(20, 30, "d")], {
            filter: (from) => from != 5 && from != 20
        }, [5, 7, 20, 30]));
        it("recognizes identical ranges", () => test([mk(0, 50, "a")], {
            add: [mk(10, 40, "a")],
            filter: () => false
        }, [0, 10, 40, 50]));
        it("skips changes", () => test([mk(0, 20, "a")], {
            changes: [[5, 15, 20]],
            filter: () => false
        }, [0, 5, 25, 30]));
        it("ignores identical sub-nodes", () => {
            let ranges = [];
            for (let i = 0; i < 1000; i++)
                ranges.push(mk(i, i + 1, "a"));
            test(ranges, {
                changes: [[900, 1000, 0]],
                add: [mk(850, 860, "b")],
                prepare: (set) => Object.defineProperty(set.chunk[0], "value", { get() { throw new Error("NO TOUCH"); } })
            }, [850, 860]);
        });
    ...
    describe("spans", () => {
        ...
        function test(set, start, end, expected) {
        ...
        it("separates the range in covering spans", () => {
            test(mkSet([mk(3, 8, "one"), mk(5, 8, "two"), mk(10, 12, "three")]), 0, 15, "3 2=one 3=one/two 2 2=three 3");
        });
        it("can retrieve a limited range", () => {
            let decos = [mk(0, 200, "wide")];
            for (let i = 0; i < 100; i++)
                decos.push(mk(i * 2, i * 2 + 2, "span" + i));
        ...
        it("doesn't get confused by same-place points", () => {
            test(mkSet([mk(1, "a"), mk(1, "b"), mk(1, "c")]), 0, 2, "1 [a] [b] [c] 1");
        });
        it("properly resyncs active ranges after points", () => {
            test(mkSet([mk(0, 20, "r1"), mk(1, 10, "r2"), mk(3, 12, { name: "p", point: true }), mk(4, 8, "r3"), mk(5, 20, "r4")]), 0, 20, "1=r1 2=r1/r2 9=[p] 8=r1/r4");
        });
    ...
    describe("between", () => {
        ...
        it("returns ranges in a zero-length set", () => {
            let set = RangeSet.of([mk(0, 0)]), found = [];
