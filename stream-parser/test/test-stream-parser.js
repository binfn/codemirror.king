...
const language = StreamLanguage.define({
    ...
    token(stream, state) {
        ...
        if (stream.match(/^\d+/))
            return "number";
        if (stream.match(/^\w+/))
            return keywords.indexOf(stream.current()) >= 0 ? "keyword" : "variableName";
        if (stream.match(/^[();{}]/))
            return "punctuation";
        stream.next();
        return "invalid";
...
describe("StreamLanguage", () => {
    it("can parse content", () => {
        ist(language.parser.parse("if (x) return 500").toString(), "Document(keyword,punctuation,variableName,punctuation,keyword,number)");
    });
    ...
    function setViewport(state, from, to) {
        let field = Language.state;
        state.field(field).context.updateViewport({ from, to });
    }
    ...
    function isNode(node, name, from, to) {
        ist(node);
