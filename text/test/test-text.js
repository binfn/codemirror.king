...
describe("Text", () => {
    ...
    it("returns the correct strings for slice", () => {
        ...
        for (let i = 0; i < 1000; i++)
            text.push("0".repeat(4 - String(i).length) + i);
        let doc = Text.of(text);
        let str = text.join("\n");
        for (let i = 0; i < 400; i++) {
    ...
    it("is iterable in reverse", () => {
        let found = "";
        for (let iter = doc0.iter(-1); !iter.next().done;)
    ...
    it("is partially iterable", () => {
        let found = "";
        for (let iter = doc0.iterRange(500, doc0.length - 500); !iter.next().done;)
            found += iter.value;
        ist(JSON.stringify(found), JSON.stringify(text0.slice(500, text0.length - 500).toString()));
    });
    it("is partially iterable in reverse", () => {
        let found = "";
        for (let iter = doc0.iterRange(doc0.length - 500, 500); !iter.next().done;)
            found = iter.value + found;
    ...
    it("can iterate over lines", () => {
        ...
        function get(from, to) {
            let result = [];
            for (let i = doc.iterLines(from, to); !i.next().done;)
                result.push(i.value);
            return result.join("\n");
        }
        ist(get(), "ab\ncde\n\n\nf\n\ng");
