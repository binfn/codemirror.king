import {parser} from "../../lezer/rust/parser.js"
...
export const rustLanguage = LRLanguage.define({
  parser: parser.configure({
    props: [
      ...
      foldNodeProp.add((type: { name: string; }) => {
        if (/(Block|edTokens|List)$/.test(type.name)) return foldInside
        if (type.name == "BlockComment") return (tree: { from: number; to: number; }) => ({from: tree.from + 2, to: tree.to - 2})
        return undefined
      }),
      styleTags({
