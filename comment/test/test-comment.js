...
import { toggleLineComment, toggleBlockComment, toggleBlockCommentByLine } from "@codemirror/comment";
import { htmlLanguage } from "@codemirror/lang-html";
describe("comment", () => {
    const defaultConfig = { line: "//", block: { open: "/*", close: "*/" } };
    /**
    ...
    ```
    */
    function s(doc, config = defaultConfig, extensions = []) {
        let markers = [], pos;
    ...
    function same(actualState, expectedState) {
    ...
    function checkToggleChain(toggle, config, docs) {
    ...
    function runLineCommentTests(k) {
        function check(...docs) {
        ...
        describe(`Line comments ('${k}')`, () => {
            ...
            it("can handle multiple selections on one line", () => {
                check(`|line| |with| |ranges|`, `${k} |line| |with| |ranges|`);
            });
